...
class ImgResizeCache
{
	...
	public function __construct($params = array())
	{
		...
		if (!file_exists($this->cache_folder))
		{
			...
			JFolder::create($this->cache_folder);
		}
		if (!file_exists($this->cache_folder.'/remote'))
		{
			//mkdir($this->cache_folder.'/remote');
			JFolder::create($this->cache_folder.'/remote');
		}
	}
	
	public function resize($imagePath, $opts)
	{
		if (substr($imagePath, 0, 7) != 'http://' && substr($imagePath, 0, 8) != 'https://' && substr($imagePath, 0, 1) != '/' && substr($imagePath, 0, strlen(JPATH_SITE)) != JPATH_SITE)	// is a relative path
			$imagePath = JPATH_SITE.'/'.$imagePath;	// fix http root mapped to a joomla subfolder with .htaccess
