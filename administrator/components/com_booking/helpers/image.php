...
/**
 * Support for upload images and create thumbnails
 * 
 * @version		$Id$
...
AImporter::helper('log');

if(ISJ3)
	include_once (JPATH_LIBRARIES . DS . 'joomla' . DS . 'archive' . DS . 'zip.php');
else
	include_once (JPATH_LIBRARIES . DS . 'joomla' . DS . 'filesystem' . DS . 'archive' . DS . 'zip.php');

//test if library GD is installed
if (!get_extension_funcs("gd"))
{
	$message = "PHP server error: GD library not installed";
	ALog::add($message);
...
class AImage
{
    ...
    function thumb($path, $cwidth = null, $cheight = null)
    {
        ...
        if (! is_file($path)) {
        	ALog::add('AImage: file '.$path.' does not exists');
            return '';
        }
        ...
        $aroot = JPath::clean(JPATH_ROOT . DS . $config->imagesCache);
        ...
        if ($data === false) {
        	ALog::add('AImage: file '.$path.' is not image');
            return '';
        }
        ...
        $rroot = JURI::root() . $config->imagesCache;
        
        if (! file_exists($aroot)) {
            if (! @mkdir($aroot, 0775, true)) {
            	ALog::add('AImage: can not create dir '.$aroot);
                return '';
            }
        }
        ...
        if (! file_exists($arthumb)) {
            if (! @mkdir($arthumb, 0775, true)) {
            	ALog::add('AImage: can not create dir '.$arthumb);
                return '';
            }
        }
